...
#             qm.setStandardButtons(QMessageBox.Ok);
    ...
    def _saveHelper( self, f, procStr, data ):
        ...
        singleNpArray= tData == type( np.array([]) ) and len( data.shape ) == 0
            
        if tData == type( str() ) or ( type( data ) == type( list() ) and _entireListIsStrings( data, type( str() ) ) ):  
    ...
    def load( self, group= DEF_GROUPSTR, name= None, default= None, returnDict= True ):
        ...
        """
        if not type( name ) == type( list() ): # name is not list
            name= [ name ]
            assert not type( default ) == type( list() ), "if name is list default must be"
            default= [ default ]
        else: # name is list
            if default is None:
        ...
        try:        
            with h5py.File( self.file, 'r', compression= self.compression ) as f:
                tmpList= []
...
def _dataConfigurationHelper( group, name, data ):
    ...
    assert type( group ) == type( str() ), "group must be string"
    if type( name ) == type( list() ):
        if dataNotNone: 
            assert type( data ) == type( list() ), "if name list, then data must be list"
    else:
        name= [ name ]
        if dataNotNone:
            data= [ data ]
...
def _entireListIsStrings( inData, inType ):
    ...
    """
    assert type( inData ) == type( list() ), "Must be list type for function"
    assert np.all( [ type( aData ) == inType for aData in inData ] )
    return True
...
def _joinGroupAndName( group, name ):
    ...
    """
    assert type( group ) == type( str() ), "group must be string...list not yet supported"
    if group == "":
        return name.strip()
    else:
